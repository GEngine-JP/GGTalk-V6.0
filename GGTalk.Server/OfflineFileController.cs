    ...
    internal class OfflineFileController
    {
        ...
        void FileController_FileResponseReceived(ESPlus.FileTransceiver.TransferingProject project, bool agree)
        {
            ...
            if (!agree) //客户端拒绝接收离线文件
            {
                ...
                //通知发送方
                var contract = new OfflineFileResultNotifyContract(project.AccepterID, project.ProjectName, false);
        ...
        {
            var accepterID = Comment4OfflineFile.ParseUserID(project.Comment);
            if (accepterID == null)
            {
                return;
            }

            var item = new OfflineFileItem();
        ...
        {
            var accepterID = Comment4OfflineFile.ParseUserID(comment);
            if (accepterID == null)
            {
                return;
            }

            var saveFileDir = AppDomain.CurrentDomain.BaseDirectory + "\\OfflineFiles\\";//根据某种策略得到存放文件的路径
        ...
        {
            var list = this.globalCache.PickupOfflineFileItem(accepterID);
            if (list != null)
            {
